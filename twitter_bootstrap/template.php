 ...
 */
function twitter_bootstrap_id_safe($string) {
 ...
 */
function twitter_bootstrap_breadcrumb($breadcrumb) {
...
function twitter_bootstrap_links($links, $attributes = array('class' => 'links'), $heading = '') {
  global $language;
  $output = '';
 ...
 */
function twitter_bootstrap_comment_wrapper($content, $node) {
 ...
 */
function twitter_bootstrap_node_submitted($node) {
...
function twitter_bootstrap_comment_submitted($comment) {
...
function twitter_bootstrap_menu_local_tasks() {
  ...
  if ($primary) {
    $output .= '<ul class="nav nav-tabs">' . $primary . '</ul>';
  }
  if ($secondary) {
    $output .= '<ul class="nav nav-pills">' . $secondary . '</ul>';
  }

  return $output;
}

function twitter_bootstrap_menu_tree($tree) {
  return '<ul class="nav nav-list">' . $tree . '</ul>';
}

function twitter_bootstrap_menu_item($link, $has_children, $menu = '', $in_active_trail = FALSE, $extra_class = NULL) {
  if($has_children)
    $icon = 'icon-plus';
  else
    $icon = 'icon-minus';
...
function twitter_bootstrap_status_messages($display = NULL) {
  $output = '';
...
function twitter_bootstrap_table($header, $rows, $attributes = array(), $caption = NULL) {
  $attributes["class"]="table";
  return theme_table($header, $rows, $attributes, $caption);
}

function twitter_bootstrap_preprocess_page(&$variables){
  $css =  theme_get_setting('twitter_bootstrap_selected_css');
  if(is_null($css)){
    variable_set("twitter_bootstrap_selected_css","bootstrap.css");
    $css = "bootstrap.css";
  }
  drupal_add_css(path_to_theme()."/bootstrap/css/".$css,'theme');
...
function twitter_bootstrap_preprocess_block(&$vars, $hook) {
  ...
  if (user_access('administer blocks')) {
    include_once './' . drupal_get_path('theme', 'twitter_bootstrap') . '/template.block-editing.inc';
    twitter_bootstrap_preprocess_block_editing($vars, $hook);
